...
REUSE_CACHE_SLOTS = {"window_cache": "window_slots", "window_cache_scale": "window_slots"}
...
def input_metadata(tokens, requests, active, case):
    ...
    lengths, prefixes = _plan(active, requests, case, "decode")
    maximum = max(prefix + length - 1 for prefix, length in zip(prefixes, lengths))
    window_pages = requests * (maximum // 128 + 1) + 1
    compressed_pages = requests * max(4, (maximum // 2) // 128 + 1) + 1
    return (
        ("x", [tokens, D], bf),
        ("wq_a", [D, Q_LORA], fp),
        ...
        ("window_indices", [tokens, 128], torch.int32),
        ("window_cache", [window_pages, 128, 1, HEAD_DIM], fp),
        ("window_cache_scale", [window_pages, 128, 1, HEAD_DIM // C.WINDOW_CACHE_GROUP], mx),
        ("compressed_cache", [compressed_pages, 128, 1, HEAD_DIM // 2], torch.uint8),
        ("compressed_cache_scale", [compressed_pages, 128, 1, HEAD_DIM // C.COMPRESSED_CACHE_GROUP], fp),
...
def build_specs(args, mode, initial_cache):
    ...
    specs = [
        ...
        for name, shape, dtype in input_metadata(
            args.tokens, args.requests, args.active_tokens, args.case
        )
    ]
...
def run_c2a_reuse(operator, mode):
    """Validate a C2A reuse production operator on A5."""
